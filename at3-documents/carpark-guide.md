...
| ---------- |-------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|
| `CarPark`    | level_row: dict, weather_message: list, emergency_message: list, community_message: list  | update_available_parking_bay_number(), get_weather_message(), get_emergency_message(), get_community_message() |
| `Sensor`     | licence_plate: list,  level_row_number: item                                              | enter_carpark(), exit_carpark(), enter_row()                                                                   |
| `Display`    | colour_display: str, weather_message: str, emergency_message: str, community_message: str | display_available_parking_bay_number(), display_message()                                                      |
...
>      `CarPark is the class responsible for the number of available bays because the information is stored in the dictionary, which was made in update_displays(), called data{}.
>    - This dictionary; data{} then calls the available_bays() method which is also in CarPark, this method uses the capacity attribute from the CarPark class. `
>    - _The current temperature_
>      `The temperature is the same as above, it is found in the data{} dictionary in CarPark, which draws information from the weather attribute in CarPark and is set when called/a new CarPark object is made at the moment.`
>    - _The time_
...
> The CarPark unit tests should include the TypeError check as that is where the register method is, so it makes sense to keep it in there where there is already a check for whether it's a Display or from Sensors.
